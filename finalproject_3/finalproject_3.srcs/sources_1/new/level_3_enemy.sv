...
module level_3_enemy(
    ...
    reg [15:0] initialstart = (1280 / (range+1))*cnt;
    reg [15:0] sprite_x     = initialstart;
    ...
    always @(posedge i_v_sync ) begin
        sprite_x <= sprite_x + (sprite_x_direction ? 2 : -2);
            
        if (sprite_x == initialstart+50) begin
            sprite_x_direction <= 0;
            sprite_x_flip <= 1;
        end
        else if (sprite_x <= initialstart-50) begin
